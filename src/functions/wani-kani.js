...
export const useWaniKaniAssignments = apiToken => {
  const { data, loading, error } = swrHook(
    useSWRImmutable(!!apiToken ? "https://api.wanikani.com/v2/assignments" : null, url =>
      fetcherNoLogging(url, { Authorization: `Bearer ${apiToken}` })
    )
  );
